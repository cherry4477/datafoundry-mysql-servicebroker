...
func (myhandler *MyCat_sharedHandler) DoProvision(instanceID string, details brokerapi.ProvisionDetails, asyncAllowed bool) (brokerapi.ProvisionedServiceSpec, ServiceInfo, error) {
	...
	json.Unmarshal(details.RawParameters, &paraMap)
	if ATTR_dbname, ok := paraMap["ATTR_dbname"]; ok {
		dbName = CheckDBname(ATTR_dbname)
	} else {
	...
	if err != nil {
		Logger.Error("Get MyCatInfo false", err)
	...
	if err != nil {
		Logger.Error("Get MySQLInfo false", err)
		return brokerapi.ProvisionedServiceSpec{}, ServiceInfo{}, err
	}
	db, err := sql.Open("mysql", mySqlDb.User+":"+mySqlDb.Password+"@tcp("+mySqlDb.Url+")/")
	defer db.Close()
	if err != nil {
		Logger.Error("Open MySQL false", err)
		return brokerapi.ProvisionedServiceSpec{}, ServiceInfo{}, err
	}
	//测试是否能联通
	err = db.Ping()
	if err != nil {
		Logger.Error("Ping MySQL false", err)
		return brokerapi.ProvisionedServiceSpec{}, ServiceInfo{}, err
	}
	...
	if err != nil {
		Logger.Error("Create database:"+dbName+" false", err)
		return brokerapi.ProvisionedServiceSpec{}, ServiceInfo{}, err
	}
	...
	if err != nil {
		Logger.Error("Grant database:"+dbName+" to user:"+newusername+" false", err)
		return brokerapi.ProvisionedServiceSpec{}, ServiceInfo{}, err
	}
...
func (myhandler *MyCat_sharedHandler) DoDeprovision(myServiceInfo *ServiceInfo, asyncAllowed bool) (brokerapi.DeprovisionServiceSpec, error) {
	...
	*/
	...
	if err != nil {
		Logger.Error("Revoke user:"+myServiceInfo.User+"from database:"+myServiceInfo.Database+" false", err)
		return brokerapi.DeprovisionServiceSpec{IsAsync: false}, err
	}
...
func getMyClusterInfo(myclusterid string) (myCluster, error) {
	...
	mycluster := myCluster{}
	if myclusterid == "" {
		Logger.Debug("Clusterid is empty")
		return mycluster, errors.New("Clusterid is empty")
	}
	data, err := ioutil.ReadFile(ConfigPath + "myclusterconfig.yaml")
	if err != nil {
		Logger.Error("Read "+ConfigPath+"myclusterconfig.yaml error", err)
	...
	if err != nil {
		Logger.Error("Parsing "+ConfigPath+"myclusterconfig.yaml error", err)
		return mycluster, err
	}
